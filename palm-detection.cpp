...
#pragma comment(lib, "E:\\workspace\\daka\\onnxruntime_RelWithDebInfo\\lib\\onnxruntime.lib")
#include <fstream>
#include <sstream>
#include <cmath>
#include <functional> 
...
int main()
{
    ...
    string testImg = rootDir + "palm_test2.jpg";
    // video info
    double fps;
    double t = 0;
    char string[10];  // 用于存放帧率的字符串
    // convert string to wchar
    std::wstring widstr = std::wstring(palmModel.begin(), palmModel.end());
    ...
    Ort::Session session(env, pwidstr, session_options);
    ...
    while(1)
    {
        ...
        //cout<<"output_tensors size"<<output_tensors.size()<<"\n";
        ...
        //cout<<"mask.sum();"<<mask.sum();
        //auto outBoxes = detectionBoxes.index(mask[0]);
        //cout<<"mask[0].sizes();"<<mask[0].sizes();        
        ...
        while(!outDetections.empty())
        {
            ...
            fpsString += string;
            putText(showFrame, fpsString, cv::Point(5, 20),cv::FONT_HERSHEY_SIMPLEX,0.5, cv::Scalar(0, 0, 0));
...
vector<torch::Tensor> weightedNMS(const torch::Tensor& detections)
{
    ...
    cout<<" remaining.sizes()" << remaining.sizes() << "  " << remaining[0] << endl;
    cout<<"detections[remaining[0]].sizes()"<<detections[remaining[0]].sizes()<<"\n";
    // torch::Tensor IoUs;
    while (remaining.size(0) > 0)
    {
        ...
        auto otherBoxes = detections.index(remaining).slice(1, 0, 4);
         cout<<"firstBox.sizes()"<<firstBox.sizes()<<"    "<<otherBoxes.sizes()<<endl;
        auto IoUs = computeIoU(firstBox, otherBoxes);
        cout << "IoUs.sizes()" << IoUs.sizes() << endl;;
        cout << "IoUs" << IoUs << endl;
